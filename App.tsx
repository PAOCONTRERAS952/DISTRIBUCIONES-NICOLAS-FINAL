...
const App: React.FC = () => {
  ...
  const handleConfirmAddToCart = (orderDetails: OrderDetails) => {
    const product = products.find(p => p.name === orderDetails.productName);
    if (!product) return;
  ...
  const handleSaveProduct = (updatedProduct: Product) => {
    ...
    // Also update selected product if it's being viewed
    if (selectedProduct && selectedProduct.id === updatedProduct.id) {
        setSelectedProduct(updatedProduct);
    }
    setIsEditModalOpen(false);
    setProductToEdit(null);
    showNotification(`${updatedProduct.name} actualizado correctamente.`);
  };
  ...
  const categories = useMemo(() => {
    const productCats = [...new Set(products.map(p => p.category))].sort();
    let baseCategories = ['Todos', ...productCats];
    
    if (products.some(p => p.isOnSale)) {
  ...
  useEffect(() => {
    ...
    filterTimeoutRef.current = window.setTimeout(() => {
      ...
      if (selectedCategory === 'Favoritos') {
        currentProducts = products.filter(p => favoriteIds.has(p.id));
      } else if (selectedCategory === 'Ofertas') {
        currentProducts = products.filter(p => p.isOnSale);
      ...
      const newlyFiltered = currentProducts.filter(p => {
  ...
  }, [selectedCategory, searchQuery, favoriteIds, view, products]);
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans">
      ...
      <a
        ...
        className="fixed bottom-6 right-6 bg-[#25D366] text-white rounded-full p-4 shadow-lg hover:bg-[#128C7E] transition-transform transform hover:scale-110 z-50 animate-whatsapp-pulse"
